...
import { createSong, deleteSong } from '@services/api/songService'
import { useArtists } from '@hooks/useArtists'
import { useAlbums } from '@hooks/useAlbums'
import { useConfirmDialog } from '@hooks/useConfirmDialog'

// Standalone Add Song form (previously mode === 'add' in unified SongForm)
export default function AddSong() {
  ...
  // Confirm dialog for delete (rarely used on add but we keep structure consistent)
  const { isOpen: isDeleteDialogOpen, data: songToDelete, closeDialog: closeDeleteDialog } = useConfirmDialog()
  ...
  const artists = [
    ...
    ...(artistsData?.map(a => ({ value: a.artistId.toString(), label: a.name })) || [])
  ]

  const albums = [
    { value: 'create-new', label: '+ Tạo album mới', isSpecial: true },
    ...(albumsData?.map(a => ({ value: a.albumId.toString(), label: a.title })) || [])
  ]

  const filteredArtists = artists.filter(artist => artist.label.toLowerCase().includes(artistSearch.toLowerCase()))
  const filteredAlbums = albums.filter(album => album.label.toLowerCase().includes(albumSearch.toLowerCase()))

  // Auto-load previews when user inputs URL
  useEffect(() => {
    if (watchedValues.coverImageUrl && !coverPreview) {
      setCoverPreview(watchedValues.coverImageUrl)
    }
  }, [watchedValues.coverImageUrl, coverPreview])

  useEffect(() => {
    if (watchedValues.audioFileUrl && !audioPreview) {
      setAudioPreview(watchedValues.audioFileUrl)
    }
  }, [watchedValues.audioFileUrl, audioPreview])
  ...
  // Extract metadata from audio URL
  const extractMetadataFromUrl = async (url) => {
    ...
    try {
      ...
      if (blob.size === 0) throw new Error('File audio trống hoặc không thể tải')
      const metadata = await parseBlob(blob)
      const hasUsefulData = metadata.common?.title || metadata.common?.artist || metadata.common?.album || metadata.format?.duration
      if (!hasUsefulData) { toast.warning('File không chứa metadata. Vui lòng nhập thủ công.', { id: 'metadata' }); return }
      let fieldsUpdated = []
  ...
  if (foundAlbum) { setValue('albumId', foundAlbum.value); fieldsUpdated.push('Album') } else { fieldsUpdated.push('Album (tên)') }
      ...
      if (metadata.common?.genre && metadata.common.genre.length > 0 && !watchedValues.genre) {
        const genreName = metadata.common.genre[0]; const foundGenre = genres.find(g => !g.isSpecial && g.label.toLowerCase() === genreName.toLowerCase())
        if (foundGenre) { setValue('genre', foundGenre.value); fieldsUpdated.push('Genre') }
      }
  ...
  // No delete action on Add page
  const handleConfirmDelete = async () => {
    try { await deleteSong(songToDelete.id); toast.success('Xóa bài hát thành công!'); navigate('/songs') } catch { toast.error('Không thể xóa bài hát') } finally { closeDeleteDialog() }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <nav className="flex items-center gap-2 text-sm text-text-secondary">
          <button onClick={() => navigate('/songs')} className="p-2 hover:bg-bg-hover rounded-lg transition-colors">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <button onClick={() => navigate('/songs')} className="hover:text-text-primary">Quản lý bài hát</button>
          <span>/</span><span className="text-text-primary">Thêm bài hát mới</span>
        </nav>
      </div>
      <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: 0.1 }} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {coverPreview ? (
                  <div className="relative group">
                    <img src={coverPreview} alt="Cover preview" className="w-full aspect-square object-cover rounded-lg" />
                  </div>
                ) : watchedValues.coverImageUrl ? (
                  <Button type="button" variant="secondary" onClick={() => { setCoverPreview(watchedValues.coverImageUrl); toast.success('Đã tải image preview') }} className="w-full">Tải image preview</Button>
                ) : null}
              </div>
            </Card>
            {/* Audio File */}
            <Card className="p-8">
              ...
              <div className="space-y-4">
                ...
                {watchedValues.audioFileUrl && (
                  <Button type="button" variant="secondary" onClick={() => extractMetadataFromUrl(watchedValues.audioFileUrl)} disabled={isExtractingMetadata} className="w-full">
                    {isExtractingMetadata ? 'Đang trích xuất...' : 'Trích xuất metadata từ audio'}
                  </Button>
                )}
                {audioPreview && (
                  <>
                    ...
                    <div className="bg-bg-secondary rounded-lg p-6">
                      <div className="flex items-center gap-4 mb-4">
                        <button type="button" onClick={togglePlayPause} className="w-12 h-12 rounded-full bg-spotify-green flex items-center justify-center hover:bg-spotify-green-dark transition-colors flex-shrink-0">
                          {isPlaying ? <Pause className="w-6 h-6 text-white" /> : <Play className="w-6 h-6 text-white ml-0.5" />}
                        </button>
                        <div className="flex-1">
                          <p className="text-text-primary font-medium">{watchedValues.title || 'Audio Preview'}</p>
                ...
                {watchedValues.audioFileUrl && !audioPreview && (
                  <Button type="button" variant="secondary" onClick={() => { setAudioPreview(watchedValues.audioFileUrl); toast.success('Đã tải audio preview') }} className="w-full">Tải audio preview</Button>
                )}
              </div>
            </Card>
          </motion.div>
          ...
          <Card className="p-8">
            ...
            <div className="flex items-center gap-6 p-6 bg-bg-secondary rounded-lg">
              <div className="w-24 h-24 rounded-lg bg-bg-tertiary flex items-center justify-center overflow-hidden flex-shrink-0">
                {coverPreview ? <img src={coverPreview} alt="Cover" className="w-full h-full object-cover" /> : <Music className="w-10 h-10 text-text-tertiary" />}
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.3 }} className="flex items-center justify-between mt-6">
          ...
          <div className="flex gap-3">
            ...
            <Button type="submit" icon={Save} disabled={isLoading}>{isLoading ? 'Saving...' : 'Save Song'}</Button>
          </div>
        </motion.div>
      </form>
